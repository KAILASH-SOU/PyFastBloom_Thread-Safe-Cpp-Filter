...
# Add parent directory to path so we can import fastbloom
...
def run_benchmark():
    ...
    try:
        ...
        print(f"Failed to connect to Redis at {redis_host}:6379. Is it running?")
        sys.exit(1)
    ...
    # Configuration
    NUM_EXISTING = 10_000
    NUM_MISSING = 90_000 # 90% missing key queries to simulate high miss environment (e.g. brute force, stale links)
    
    # Initialize our fast C++ Bloom Filter
    # Size 1,000,000 bits, 7 hashes
    ...
    # Create the workload mix randomly shuffled
    import random
    ...
    # --- REDIS ONLY BENCHMARK ---
    start_time_redis = time.time()
    redis_hits = 0
    redis_misses = 0
    
    # Sequential queries (In real life this would be concurrent, but sequential highlights latency per call)
    for key in workload:
        val = r.get(key)
        if val:
            redis_hits += 1
    ...
    # --- BLOOM + REDIS BENCHMARK ---
    start_time_bloom = time.time()
    bloom_hits_db_hits = 0
    bloom_hits_db_misses = 0 # False positives
    bloom_misses = 0 # Prevented DB query
    
    for key in workload:
        if bf.contains(key):
            # Bloom filter says YES, so we must query the DB
            val = r.get(key)
            if val:
            ...
            # Bloom filter says NO, DB query PREVENTED
            bloom_misses += 1
